...
class DeviceRemoteDataSource extends DeviceDataSource {
  ...
  Future<ChiscoResponse> addPower(AddPower power) async {
    ChiscoResponse response =
        await client.request(url: 'device/save', data: power.toJson());
    // print(response);
    return response;
  }
  ...
  Future<ChiscoResponse> deleteDevice(String serialNumber) async {
    ChiscoResponse response = await httpClient
        .request(url: 'device/delete', data: {'serialNumber': serialNumber});
    return response;
  }
}
